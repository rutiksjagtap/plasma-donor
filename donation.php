...
<br>
<h1 align="center">Recipient List</h1>
...
<form class="" action="" method="post">
  ...
  &nbsp;<label style="font-size:20px">  Enter ID of Recipient:</label>
  <input type="text" name="pid" placeholder="ID of Recipient" style="font-size: 22px; width: 220px; height: 40px; border-radius: 10px;">
<br>
...
$qp=$db->query("SELECT * FROM recipient WHERE id='$pid'");
...
$date=$current;
 if($bdgroup==$plgroup){
  $t=$db->prepare("INSERT INTO donation(donationid,dname,pname,plgroup,date) VALUES (:donationid,:dname,:pname,:plgroup,:date)");
  $t->bindValue('donationid',$donationid);
  $t->bindValue('dname',$dname);
  $t->bindValue('pname',$pname);
  $t->bindValue('plgroup',$plgroup);
...
$u=$db->prepare("UPDATE `donor` SET `date`='$date' WHERE id='$did'");
  ...
  echo "<script>alert('Plasma Group not matched!')</script>";
}} 
else
{
